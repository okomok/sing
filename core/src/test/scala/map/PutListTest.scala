...
class PutListTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testTrivial {
        ...
        free.assertSame[l, m#asList#force]
        assertEquals(l, m.asList)
   }

    def testTrivialNil {
        type m   = map.sorted[nat.naturalOrdering]#put[_8, _9]#putList[Nil]
        val m: m = map.sorted(nat.naturalOrdering).put(_8, _9).putList(Nil)
