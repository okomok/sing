...
class EqualWithTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testTrivial {
        ...
        type m2   = SortedMap.empty[Nat.kind.naturalOrdering]#put[_8, _9]#putList[xs2]
        val m2: m2 = SortedMap.empty(Nat.kind.naturalOrdering).put(_8, _9).putList(xs2)

        Test.assertSame[`true`, m#equalWith[m, Nat.kind.naturalOrdering]]
        Test.assertSame[`true`, m#equalWith[m2, Nat.kind.naturalOrdering]]
        Test.assertSame[`true`, m2#equalWith[m, Nat.kind.naturalOrdering]]
        assertEquals(`true`, m.equalWith(m2, Nat.kind.naturalOrdering))
    }
    ...
    def testTrivialDifferentKey {
        ...
        Test.assertSame[`true`, m#equalWith[m, Nat.kind.naturalOrdering]]
        Test.assertSame[`false`, m#equalWith[m2, Nat.kind.naturalOrdering]]
        Test.assertSame[`false`, m2#equalWith[m, Nat.kind.naturalOrdering]]
        assertEquals(`false`, m.equalWith(m2, Nat.kind.naturalOrdering))
    }
